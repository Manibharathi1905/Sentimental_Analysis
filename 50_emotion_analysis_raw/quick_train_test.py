...
Usage:
  python src/quick_train_test.py --config configs/config.yaml

This script:
 - loads a DistilBERT model for multi-label classification (20 emotions)
 - trains on GoEmotions + EmpatheticDialogues with weighted BCEWithLogitsLoss for ~10 minutes
 - evaluates on validation set and prints metrics
"""
...
def quick_train_test(config_path):
    ...
    # Use only 10% of training data for quick test to reduce time
    train_df = train_df.head(int(len(train_df) * 0.1))

    print(f"Training samples (subset): {len(train_df)}, Validation samples: {len(val_df)}")
    ...
    print("Starting quick training test for ~10 minutes...")
    
    model.train()
    total_loss = 0
    batch_count = 0
    for batch in tqdm(train_loader, desc="Quick Training"):
        optimizer.zero_grad()
        ...
        if time.time() - start_time > time_limit:
            print(f"Time limit reached after {batch_count} batches.")
            break
    
    avg_loss = total_loss / batch_count if batch_count > 0 else 0
    print(f"Average loss after quick training: {avg_loss:.4f}")
    ...
    print(f"Validation metrics after quick training: {metrics}")
